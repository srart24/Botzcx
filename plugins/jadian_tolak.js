...
let handler = async (m, { conn, text }) => {
  ...
  if(!text && !m.quoted) return conn.reply(m.chat, `*Tag atau reply chat Yang Ingin Anda Tolak Cintanya.*`, m)
   ...
   // if(user === conn.user.jid) return conn.reply(m.chat, `*Tidak bisa berpacaran dengan bot.*`, m)
    ...
    if(db.data.users[user].pasangan != m.sender){
      ...
      db.data.users[user].pasangan = ""
       m.reply(`*Anda baru saja menolak @${user.split('@')[0]} awokawok*`,null, { mentions: [user]})
    }
	}	
}
