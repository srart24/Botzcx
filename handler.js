    ...
    try {
        ...
        try {
            ...
            if (user) {
                ...
                if (!isNumber(user.lastngeped))
                    user.lastngeped = 0
                if (!isNumber(user.lilin))
                    user.lilin = 0
                if (!isNumber(user.korek))
                    user.korek = 0
                ...
                if (!isNumber(user.lastclaim))
                    ...
                    // Memancing By; @srart24
                    
                    if (!isNumber(user.buntal))
                    user.buntal = 0
                    if (!isNumber(user.teri))
                    user.teri = 0
            ...
            if (settings) {
                ...
                if (!('games' in settings)) settings.games = false
        ...
        for (let name in plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (m.chat in db.data.chats || m.sender in db.data.users) {
                    let chat = db.data.chats[m.chat]
                    let user = db.data.users[m.sender]
                    if (name != 'ow_unbanchat.js' && chat?.isBanned)
                        return // Except this
                    if (name != 'ow_unbanuser.js' && user?.banned)
                        return
                }
                ...
                if (!isPrems && plugin.limit && db.data.users[m.sender].limit < plugin.limit * 1) {
                   this.sendButton(m.chat, 'Limit Kamu Habis >_<', wm, [['Beli', '.topup']], m)
            ...
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || Connection.conn.welcome || 'Hallo Kak \n\nWelkom to Grup @subject\nJangan Lupa Intro Dulu Ygy').replace('@beb', await this.getName(user)).replace('@subject', await this.getName(id)).replace('@desc', await this.groupMetadata.desc) :
                            (chat.sBye || this.bye || Connection.conn.bye || 'Byeeee Semoga Tenang Di Sana')).replace('@beb', await this.getName(user)).replace('@user', '@' + user.split('@')[0])
                       let jgp = `Nama Group : ${groupMetadata.subject}
Jumlah Anggota : ${groupMetadata.participants.length}`
                    await this.sendHydrated(id, text.trim(), jgp, pp, 'https://bit.ly/3OixqH5', ' Instagram ', null, null, [[ 'Y' , 'y' ]])
                    }
                }
            }
            break
        ...
        case 'promote':
            text = (chat.sPromote || this.spromote || Connection.conn.spromote || '*🥳 Selamat Kepada @user Sekarang Kamu Adalah Admin 🥳*')
        case 'demote':
            if (!text)
                text = (chat.sDemote || this.sdemote || Connection.conn.sdemote || '*Jiahaha @user Turun Jabatan Jadi Babu Grup Awokawok*')
    ...
    if (message.keys && Array.isArray(message.keys)) {
        try {
            for (const key of message.keys) {
                ...
                await this.reply(msg.key.remoteJid, `Terdeteksi @${participant.split`@`[0]} telah menghapus pesan`.trim(), msg, {
